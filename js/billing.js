...
// FIXED: Update totals using restaurant settings - CORRECTED SELECTOR
function updateTotals() {
    ...
    // Update labels - SIMPLE APPROACH
    // Look for GST label - check all spans
    document.querySelectorAll('span').forEach(span => {
        const text = span.textContent || '';
        if (text.includes('GST') && text.includes('%')) {
            span.textContent = `GST (${gstRate}%)`;
        }
        if (text.includes('Service Charge') && text.includes('%')) {
