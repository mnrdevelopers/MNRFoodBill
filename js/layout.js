...
// Add to layout.js for better mobile experience
function optimizeMobileTables() {
    ...
    tableWrappers.forEach(wrapper => {
        ...
        const buttons = wrapper.querySelectorAll('.action-btn, button');
        buttons.forEach(btn => {
...
document.addEventListener('DOMContentLoaded', function() {
    setTimeout(optimizeMobileTables, 500);
});

// Refresh tables on orientation change
window.addEventListener('orientationchange', function() {
    setTimeout(() => {
        if (window.TableScroll && window.TableScroll.refresh) {
            window.TableScroll.refresh();
        }
